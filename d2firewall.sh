...
NETWORK=$(cat network.txt 2>/dev/null || echo "10.8.0.1/24")
RED='\033[0;31m'
GREEN='\033[0;32m'
BLUE='\033[0;34m'
NC='\033[0m'
...
reset_ip_tables () {
  ...
  if ( ip a | grep -q "tun0" ) && [ "$INTERFACE" == "tun0" ]; then
    if ! iptables-save | grep -q "POSTROUTING -s 10.8.0.1/24"; then
...
if [ "$action" == "setup" ]; then
  ...
  wget -q https://raw.githubusercontent.com/fswdevl/Destiny-2-Matchmaking-Firewall/main/d2firewall.sh -O ./d2firewall.sh
